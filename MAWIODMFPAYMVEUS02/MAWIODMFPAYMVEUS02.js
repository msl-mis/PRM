...
function CustomerSaveCheck_Head(tStatus)
{
	...
	if (tStatus == "CREATE")
	{
		...
		//Radio Button 驗証
			...
			{
				tErr += getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'chpay_Err', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+ '\r\n';
			}
		//20230424 Peggy Star
		//驗證第二個開窗必填
	...
	if(tdatetime1.length>0){
		...
		if (tdatetime1Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'datetime1', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(tdept.length>0){
		...
		if (tdeptValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'dept', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(tinv01.length>0){
		...
		if (tinv01Value.length>0){
			if(!/^-?\d+$/.test(tinv01Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'inv01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(tinv02.length>0){
		...
		if (tinv02Value.length>0){
			if(!/^-?\d+$/.test(tinv02Value)){
				//輸入的資料不符合int數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'inv02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tIntErrMsg + '\r\n';
			}
		}
	}
	...
	if(tmoney01.length>0){
		...
		if (tmoney01Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'money01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(tmoney01.length>0){
		...
		if (tmoney01Value.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney01Value)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'money01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(tmoney02.length>0){
		...
		if (tmoney02Value.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney02Value)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'money02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(tmoney03.length>0){
		...
		if (tmoney03Value.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmoney03Value)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'money03', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(tmtotal.length>0){
		...
		if (tmtotalValue.length>0){
			if(!/^-?(?:\d+|\d{1,3}(?:,\d{3})+)(?:\.\d+)?$/.test(tmtotalValue)){
				//輸入的資料不符合float數值格式 !
				tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'mtotal', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFloatErrMsg + '\r\n';
			}
			else{
	...
	if(tpayee.length>0){
		...
		if (tpayeeValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'payee', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(ttextarea1.length>0){
		...
		if (ttextarea1Value.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'textarea1', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if(tusername.length>0){
		...
		if (tusernameValue.length==0){
			//欄位不允許空白 !
			tErr += '「' + getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'username', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx') + '」  ' + tFieldNotFilledMsg + '\r\n';
		}
	}
	...
	if($('#'+pNumberContrlID+'_txt').length>0 && $('#'+pNumberContrlID+'_txt').is(':visible')){
		...
		if(tControlNumValue.length>0){
			if(isNaN(tControlNumValue)) {
				...
				tControlNumValue = tw.com.dsc.easyflowDotNet.program.MAWIODMFPAYMVEUS02.ajaxGetFixedNum(tControlNumValue, pDecimalPlaces).value;
			...
			var tControlWordValue = tw.com.dsc.easyflowDotNet.program.MAWIODMFPAYMVEUS02.ajaxGetMoneyWord(tControlNumValue).value;
	...
	if($("#chkatt01_chk").length>0){
		if($("#chkatt01_chk")[0].checked){
			if($("#inv01_txt").val().trim().length==0){
				tErr+='[inv01-'+getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'inv01', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#inv01_txt").val('');
	...
	if($("#chkatt02_chk").length>0){
		if($("#chkatt02_chk")[0].checked){
			if($("#inv02_txt").val().trim().length==0){
				tErr+='[inv02-'+getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'inv02', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#inv02_txt").val('');
	...
	if($("#chkitem05_chk").length>0){
		if($("#chkitem05_chk")[0].checked){
			if($("#chkother_txt").val().trim().length==0){
				tErr+='[chkother-'+getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'chkother', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#chkother_txt").val('');
	...
	if($("#other_chk").length>0){
		if($("#other_chk")[0].checked){
			if($("#attother_txt").val().trim().length==0){
				tErr+='[attother-'+getI18NForSpecial('FD', 'MAWIODMFPAYMVEUS02', 'attother', '../../_Common/PlatformUtil/KernelPage/I18N/I18NForJs.aspx')+']'+tFieldNotFilledMsg+'\r\n';
			}
		}
		else{
			$("#attother_txt").val('');
...
function jsDoDispatch(pFormID, pSheetNo, pDispatchFormID){
	...
	if(window.confirm(tConfirmSTR.split('[')[0].toString() + "\r\n\r\n" + tConfirmSTR.split('[')[1].toString())){
		var bResult = tw.com.dsc.easyflowDotNet.program.MAWIODMFPAYMVEUS02.ajaxDoDispatchForm(pFormID, pSheetNo, pDispatchFormID).value;
